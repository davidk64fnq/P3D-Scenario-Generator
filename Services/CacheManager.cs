    ...
    public class CacheManager(ILogger log) : ICacheManager
    {
        ...
        public async Task<bool> TrySerializeToFileAsync<T>(T data, string filePath, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                {
                    await _log.InfoAsync($"Creating directory '{directory}'.");
        ...
        public async Task<(bool success, T data)> TryDeserializeFromFileAsync<T>(string filePath, CancellationToken cancellationToken = default)
        {
            ...
            {
                await _log.WarningAsync($"The specified file was not found: '{filePath}'.");
            ...
            {
                await _log.ErrorAsync($"Error deserializing data from file '{filePath}'. Details: {ex.Message}", ex);
                return (false, default);
            }
        }
    }
