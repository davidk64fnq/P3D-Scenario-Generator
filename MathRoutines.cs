namespace P3D_Scenario_Generator
{
    class MathRoutines
    {
        static internal void AdjCoords(double dStartLat, double dStartLon, double dHeading, double dDist, ref double dFinishLat, ref double dFinishLon)
        {
            ...
            double dLat2 = Math.Asin(Math.Sin(dLat1) * Math.Cos(dDist / Con.radiusEarth) + Math.Cos(dLat1) * Math.Sin(dDist / Con.radiusEarth) * Math.Cos(dBrng));
            ...
            double dLon2 = dLon1 + Math.Atan2(Math.Sin(dBrng) * Math.Sin(dDist / Con.radiusEarth) * Math.Cos(dLat1), Math.Cos(dDist / Con.radiusEarth) - (Math.Sin(dLat1) * Math.Sin(dLat2)));
